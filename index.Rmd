---
title: Neighbours shapes pollination network topology
output:
    rmdformats::readthedown:
      collapsed: false
      highlight: kate
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyverse)
library(glmmTMB)
library(bipartite)
library(igraph)
library(vegan)
library(rnetcarto)
library(sjPlot)
library(corrplot)
```



### Build network from visitation dataset

```{r, loading clean datasets}
#dataset with aggregated visits (i.e. number of visits per rep, no pollinator ID) with joined covariates & calculated densities
#contains reps with zero visits
#wrangling all done using visits.R

data <- read.csv("Data/Output/visitation_cleaned.csv")


#long version with pollinator ID, list of visits only for building network 
visits <- read.csv("Data/visitation_data.csv")
visits$Quantity <- as.numeric(as.character(visits$Quantity))
visits <- dplyr::filter(visits, Quantity>0)
visits$Site <- "site"
visits$ID <- gsub(" ","", visits$ID)
visits$ID <- gsub('\"',"", visits$ID)
visits$Species <- gsub(" ","", visits$Species)
visits$Species <- gsub('\"',"", visits$Species)
sum(visits$Quantity)
visits$uniID <- paste(visits$Species, visits$WP)
```

# Functions
```{r, functions}

#this function creates an adjencancy matrix from the long visit only dataset
#uniID - plant ID, can use to create species/individual/RTU/functional groups networks from dataset
#y is bottom - uniID or plant Species
#z is top - pollinator RTU or functional group
net <- function(x, y, z){
  y <- enquo(y)
  z <- enquo(z)
  fnet <- dplyr::select(x, !!y, !!z, Site, Quantity)
  fnet$Quantity <- as.numeric(fnet$Quantity)
  print(sum(fnet$Quantity))
  long.ag <- fnet %>% group_by(!!y, !!z) %>% summarise(Quantity = sum(Quantity)) 
  print(sum(long.ag$Quantity))
  wide <- spread(long.ag, !!z, Quantity)
  wide[is.na(wide)] <- 0
  wide <- as.data.frame(wide)
  rownames(wide) = wide[,1 ] # the first row will be the header
  rownames(wide) <- wide[,1]
  wide <- select(wide, -!!y)
}

#calculates z-scores
net.zscore = function(obsval, nullval) {
  (obsval - mean(nullval))/sd(nullval)  
}

#creates random network using vegan with fixed marginal totals. Creates 1000 and puts in one big dataframe
ranindices <- function(x){
  y <- permatfull(x, fixedmar = "both", times = 1000)
  y <- y$perm
  y_indices <- map(y, networklevel)
  ran <-  do.call("rbind", y_indices)
  ran <- as.data.frame(ran)
}

#calculates the modularity of 1000 random networks with fixed marginal totals of input and puts in a big dataframe. Bipartite modularity 
ranModules <- function(x){
  y <- permatfull(x, fixedmar = "both", times = 1000)
  y <- y$perm
  z <- list()
  null.mod <- map(y, computeModules)
  for (i in 1:1000){
    ind <- null.mod[[i]]@likelihood
    z <- c(z, ind)
  } 
  do.call(rbind, lapply(z, as.data.frame))
}


```
#  Calculations

## Centrality measures
```{r}
#make an individual network using functional groups
ind1 <- net(visits, uniID, fun.grp)

#project into a unipartite network
one1 <- as.one.mode(ind1, fill = 0, project = "lower", weight = "TRUE")

#calculate centrality measures
one1 <- graph_from_adjacency_matrix(one1)
cls1 <- closeness(one1)
ei1 <- eigen_centrality(one1)
bt1 <- betweenness(one1)
dg1 <- degree(one1)

#cbind and join to main covariate dataset
dat1 <- as.data.frame(cbind(cls1, ei1$vector, bt1, dg1))
dat1$uniID <- row.names(dat1)
all <- left_join(dat1, data, by = "uniID")

#betweenness is zero inflated so a binary variable
all <- mutate(all, btbin = ifelse(bt1 == 0, 0, 1))

#filter out the over zero part to model separately
bt <- filter(all, bt1 > 0)
```

## Effective partners
```{r, specieslevel indices}
#calculate species level indices
sp <- specieslevel(ind1, index = "effective partners")
pl <- sp[2]
pl <- as.data.frame(pl)
pl$uniID <- row.names(pl)
all <- left_join(pl, all, by = "uniID")
```

## Proportion of conspecific/heterospecific interactors

```{r, population proportions}

#unweighted network
un <- as.one.mode(ind1, fill = 0, project = "lower", weight = "FALSE")

#need to subset
un <- as.data.frame(un)
un$uniID <- row.names(un)
dat <- left_join(all, un, by = "uniID") 
dat <- select(dat, -(18:28))
#calculate number of conspecific interactors
dat <- mutate(dat, cons = ifelse(Species == "PP", rowSums(select (dat, starts_with("PP"))), +
  ifelse(Species == "SC", rowSums(select(dat, starts_with("SC"))),+    ifelse(Species == "HH", rowSums(select(dat, starts_with("HH"))),  +      ifelse(Species == "LT",rowSums(select(dat, starts_with("LT"))), +       ifelse(Species == "AS", rowSums(select(dat, starts_with("AS"))), +      ifelse(Species == "EC", rowSums(select(dat, starts_with("EC"))), +      ifelse(Species == "BW", rowSums(select(dat, starts_with("BW"))), +        ifelse(Species == "EL", rowSums(select(dat, starts_with("EL"))), +  ifelse(Species == "SM", rowSums(select(dat, starts_with("SM"))),
  + ifelse(Species == "SD", rowSums(select(dat, starts_with("SD"))), 0)))))))))))

```

```{r}
#calculate heterospecific interactors

dat <- mutate(dat, hets = ifelse(Species == "PP", rowSums(select(dat, 37:188, 207:265)), +
   ifelse(Species == "SC", rowSums(select(dat, 256:265, 37:206)),+    
 ifelse(Species == "HH", rowSums(select(dat, 37:131, 136:265)),  +      ifelse(Species == "LT",rowSums(select(dat, 37:135, 189:265)), +       ifelse(Species == "AS", rowSums(select(dat, 78:265)), +          ifelse(Species == "EC", rowSums(select(dat, 37:96, 131:265)), +      ifelse(Species == "BW", rowSums(select(dat, 37:77, 96:265)), +        ifelse(Species == "EL", rowSums(select(dat, 37:130, 132:265)), +  ifelse(Species == "SM", rowSums(select(dat, 37:261)),
 + ifelse(Species == "SD", rowSums(select(dat, 37:255, 262:265)), 0)))))))))))


#standardize by abundances
dat <- select(dat, -(37:265))
abun <- dat %>% group_by(Species) %>% count()
dat <- left_join(dat, abun, by = "Species")
dat <- mutate(dat, hetn = 231 - n)
dat <- mutate(dat, st.con = cons/n)
dat <- mutate(dat, st.het = hets/hetn)
dat <- select(dat, uniID, 37:42)
all <- left_join(all, dat, by = "uniID")
```

## Contribution to nestedness
```{r, nested}
#this step is slow & can be skipped w/o issues. load from csv but code is here.  takes 30 min 
#ne <- nestedcontribution(ind1)
#ne.low <- ne[[2]]
#ne.top <- ne[[1]]
#ne.low$uniID <- row.names(ne.low)
#write.csv(ne.low, "nelow.csv")
#write.csv(ne.top, "netop.csv")
ne.low <- read.csv("nelow.csv")
ne.low <- select(ne.low, -X)
all <- left_join(all, ne.low, by = "uniID")
```

## Unipartitite modularity
```{r, unipartite modularity}
one1 <- as.one.mode(ind1, fill = 0, project = "lower", weighted = TRUE)
mods <- netcarto(one1)
n1data <- mods[[1]]
n1data <- rename(n1data, uniID = name)
all <- left_join(all, n1data, by = "uniID")
all$module <- as.factor(all$module)
```



# EDA


```{r}
labels = c("Acamptopappus sphaerocephalus", "Eriogonum fasciculatum", "Ericameria cooperi", "Ericameria linearifolia", "Larrea tridentata", "Scutellaria mexicana", "Salvia dorri", "Echinocereus engelmannii", "Opuntia basilaris", "Cylindropuntia echinocarpa")

ggplot(data, aes(N.flowers, Quantity)) + geom_point(aes(color = Species)) + geom_smooth(method = lm, se = FALSE, color = "black") + scale_color_discrete("", labels = labels) + ylab("Visitation Rate") + xlab("Floral display size")

ggplot(data, aes(N.flowers, Height)) + geom_point(aes(color = Species)) + geom_smooth(method = lm, se = FALSE, color = "black") + scale_color_discrete("", labels = labels) + xlab("Floral display size")


ggplot(data, aes(shrub.density, Quantity)) + geom_point(aes(color = Species)) + geom_smooth(method = lm, se = FALSE, color = "black") + scale_color_discrete("", labels = labels) + ylab("Visitation rate") + xlab("Neighbourhood shrub density")

pl <- select(all, Species, shrub.density, dg1, st.con, st.het)
pl <- gather(pl,type, prop, 4:5)

ggplot(pl, aes(dg1, prop, color = type)) + geom_point()  +xlab("Degree Centrality") + ylab("Proportion of Population")

ggplot(pl, aes(shrub.density, prop, color = type)) + geom_point() + geom_smooth(method = lm) +xlab("Shrub Density") + ylab("Proportion of Population")

c <- select(all, 3:6, Quantity)
c <- rename(c, closeness = cls1, eigancentrality = V2, betweenness = bt1, degree = dg1, visitation.rate = Quantity)
M <- cor(c)
corrplot(M, method = "number") # Display the correlation coefficient
```

# Modelling

```{r, visitation rates}
m1 <- glmmTMB(Quantity ~density + N.flowers + imp.den + (1|Species), family = "nbinom2", data = data)       

m1p <- glmmTMB(Quantity ~density + N.flowers + imp.den + (1|Species), family = "poisson", data = data)       

AIC(m1, m1p)
#negative binomial #2 is the best

m1 <-glmmTMB(Quantity ~density * N.flowers + day + (1|Species), family = "nbinom2", data = data)
m2 <- glmmTMB(Quantity ~ density + N.flowers + day + (1|Species), family = "nbinom2", data = data)       
m3 <- glmmTMB(Quantity ~ density + N.flowers * day + (1|Species), family = "nbinom2", data = data) 
m4 <-glmmTMB(Quantity ~shrub.density * N.flowers + day + (1|Species), family = "nbinom2", data = data)
m5 <- glmmTMB(Quantity ~ shrub.density + N.flowers + day + (1|Species), family = "nbinom2", data = data)       
m6 <- glmmTMB(Quantity ~ shrub.density + N.flowers * day + (1|Species), family = "nbinom2", data = data)
m7 <- glmmTMB(Quantity ~ shrub.density + N.flowers * imp.den + (1|Species), family = "nbinom2", data = data)
m8 <- glmmTMB(Quantity ~ cactus.density + N.flowers * day + (1|Species), family = "nbinom2", data = data)
m9 <- glmmTMB(Quantity ~ con.density + N.flowers * day+ (1|Species), family = "nbinom2", data = data)

mnull <-glmmTMB(Quantity ~ (1|Species), family = "nbinom2", data = data)

kable(AIC(m1, m2, m3, m4, m5, m6, m7, m8, m9, mnull))
summary(m6)
tab_model(m6)
anova(m6, mnull)
kable(car::Anova(m6, type = 3))

library(performance)
check_collinearity(m6)
```


```{r, centrality measures}
#degree centrality

ggplot(all, aes(dg1)) + geom_density()
m2 <- glmmTMB(dg1 ~ N.flowers+ Quantity + (1|Species), family = "nbinom2", data = all)
m3 <- glmmTMB(dg1 ~ N.flowers+ Quantity + (1|Species), family = "poisson", data = all)
m4 <- glmmTMB(dg1 ~ N.flowers+ Quantity + (1|Species), family = "nbinom1", data = all)
AIC(m2, m3, m4)
#nbinom2

m5 <- glmmTMB(dg1 ~ N.flowers +(1|Species), family = "nbinom2", data = all)
m6 <- glmmTMB(dg1 ~ Quantity+(1|Species), family = "nbinom2", data = all)
m7 <- glmmTMB(dg1 ~ Quantity+ shrub.density + day  + N.flowers + (1|Species), family = "nbinom2", data = all)
summary(m7)
m8 <- glmmTMB(dg1 ~ Quantity+ shrub.density + day  * N.flowers + (1|Species), family = "nbinom2", data = all)
summary(m8)
mnull <- glmmTMB(dg1 ~  (1|Species), family = "nbinom2", data = all)

AIC(m2, m5, m6, m7, m8, mnull)

summary(m6)
car::Anova(m6, type = 2)

kruskal.test(dg1 ~ Species, data = all)


ggplot(all, aes(Species, dg1)) + geom_boxplot() + ylab("Degree Centrality")


#betweeness
ggplot(all, aes(bt1)) + geom_density()
b1 <- glmmTMB(btbin ~ shrub.density + Quantity + (1|Species), family = "binomial", data = all)
summary(b1)
car::Anova(b1, type = 2)

b2 <-glmmTMB(btbin ~ shrub.density + Quantity + day + N.flowers + (1|Species), family = "binomial", data = all)
summary(b2)
car::Anova(b2, type = 2)

b3 <- glmmTMB(log(bt1) ~ Quantity +(1|Species), family = "gaussian", data = bt)
summary(b3)
car::Anova(b3, type = 2)
shapiro.test(resid(b3))

b4 <- glmmTMB(log(bt1) ~ Quantity + shrub.density + day + N.flowers + (1|Species), family = "gaussian", data = bt)
summary(b4)
car::Anova(b4, type = 2)

#Eigancentrality
ggplot(all, aes(V2)) + geom_density()
shapiro.test(log(all$V2))

v1 <- glmmTMB(V2 ~ day + Quantity + (1|Species), family = "gaussian", data = all)
summary(v1)
car::Anova(v1, type = 2)
shapiro.test(resid(v1))

v2 <- glmmTMB(V2 ~ Quantity+ shrub.density + N.flowers + day +(1|Species), family = "gaussian", data = all)
summary(v2)
car::Anova(v2, type = 2)


v3 <- glmmTMB(V2 ~  Quantity + (1|Species), family = "gaussian", data = all)
summary(v3)
car::Anova(v3, type = 2)
AIC(v1, v2, v3)

shapiro.test(resid(v3))
vnull <-glmmTMB(V2 ~ (1|Species), family = "gaussian", data = all)
AIC(v1, vnull)
anova(v1, vnull)

kruskal.test(V2 ~ Species, data = all)
ggplot(all, aes(Species, V2)) + geom_boxplot() + ylab("Eigancentrality")

```

#maybe just use the number of sampled plants as an offset??
#conspecifics
```{r}
all <- mutate(all, uncon = n - cons)

y <- cbind(all$cons, all$uncon)



m4 <- glmmTMB(y ~ shrub.density+ dg1* Quantity+ (1|Species), family = "binomial"(link = "logit"), data = all)
summary(m4)
#interact_plot(m4, dg1, Quantity)
overdisp(m4)

m5 <- glmmTMB(y ~ shrub.density+ dg1+Quantity+ (1|Species), family = "binomial"(link = "logit"), data = all)
AIC(m4, m5)
car::Anova(m4, type = 3)
summary(m4)

m5 <- glm(y ~ day+dg1 + shrub.density + Species, family = "quasibinomial"(link = "logit"), data = all)
car::Anova(m5, type = 2)
summary(m5)

m4 <- MASS::glmmPQL(y ~ shrub.density+ dg1, random = ~1|Species, family = "quasibinomial"(link = "logit"), data = all)
summary(m4)

m5 <- MASS::glmmPQL(y ~ shrub.density+ dg1 * Quantity, random = ~1|Species, family = "quasibinomial"(link = "logit"), data = all)
summary(m5)
car::vif(m5)
car::Anova(m5, type = 2)

library(jtools)



cor.test(all$shrub.density, all$dg1)

#heterospecific access
all <- mutate(all, unhet = hetn - hets)
z <- cbind(all$hets, all$unhet)

m1 <- glmmTMB(z ~ day + shrub.density+ dg1+ Quantity+ N.flowers+ (1|Species), family = "binomial"(link = "logit"), data = all)
summary(m1)
overdisp(m1)

m5 <- MASS::glmmPQL(z ~ dg1 * Quantity, random = ~1|Species, family = "quasibinomial"(link = "logit"), data = all)
summary(m5)
car::Anova(m5, type = 3)

car::vif(m5)

cor.test(all$dg1, all$Quantity)
```

#Effective partners
```{r}


m1 <- glmmTMB(effective.partners ~ Quantity+day + shrub.density + N.flowers + (1|Species), family = "poisson", data = all)

summary(m1)


m2 <- glmmTMB(effective.partners ~ Quantity + shrub.density + day+ (1|Species), family = "poisson", data = all)

m3 <- glmmTMB(effective.partners ~ Quantity + shrub.density + (1|Species), family = "poisson", data = all)

summary(m3)
car::Anova(m3, type = 2)
overdisp(m3)

m4 <- glmmTMB(effective.partners ~ Quantity + shrub.density + day+ N.flowers + (1|Species), family = "poisson", data = all)
summary(m4)
car::Anova(m4, type = 2)

mnull <- glmmTMB(effective.partners ~ (1|Species), family = "poisson", data = all)

AIC(m2, m3, m4, mnull)

ggplot(all, aes(Species, effective.partners)) + geom_boxplot()
kruskal.test(effective.partners ~ Species, data = all)
```

```{r, nestedness}
ggplot(all, aes(nestedcontribution)) + geom_density()
shapiro.test(all$nestedcontribution)

n1 <- glmmTMB(nestedcontribution ~ N.flowers + Quantity + day + shrub.density + +(1|Species), family = "gaussian", data = all)
shapiro.test(resid(n1))
summary(n1)
car::Anova(n1, type = 2)

n2 <- glmmTMB(nestedcontribution ~ Quantity  +(1|Species), family = "gaussian", data = all)
shapiro.test(resid(n2))
summary(n2)
car::Anova(n2, type = 2)

a1 <- lm(nestedcontribution ~ Species, data = all)
shapiro.test(resid(a1))
summary(a1)
```



Network comparisons

{r}
sp <- net(visits, Species, fun.grp)
n <- networklevel(sp)
n <- as.data.frame(n)

i <- as.data.frame(networklevel(ind1))
sp.m <- computeModules(sp)
computeModules(ind1)

sp.ran.mod <- ranModules(sp)
mean(sp.ran.mod$`X[[i]]`)
net.zscore(sp.m@likelihood, sp.ran.mod$`X[[i]]`)
ind.ran.mod <- ranModules(ind1)
#ind.ran.mod <- read.csv("randommoduleoutput.csv")
net.zscore(0.5893211, ind.ran.mod$t)
mean(ind.ran.mod$t)
max(ind.ran.mod$t)




#ind.WNODF <- ranindices(ind1)
#net.zscore(4.65012620, ind.WNODF$`weighted NODF`)
#mean(ind.WNODF$`weighted NODF`)
max(ind.WNODF$`weighted NODF`)
min(ind.WNODF$`weighted NODF`)

#ind.WC <- ranindices(ind1)
#net.zscore(0.08094487, ind.WC$`weighted connectance`)
mean(ind.WC$`weighted connectance`)
max(ind.WC$`weighted connectance`)
min(ind.WC$`weighted connectance`)

write.csv(ind.WC, "indrandomWC.csv")

#ind.H2 <- ranindices(ind1)
#net.zscore(0.56665181, ind.H2$H2)
mean(ind.H2$H2)
max(ind.H2$H2)

write.csv(ind.H2, "indrandomH2.csv")

ran.sp <- ranindices(sp)
net.zscore(0.42532319, ran.sp$H2)
mean(ran.sp$H2)
max(ran.sp$H2)

net.zscore(32.82754538, ran.sp$`weighted NODF`)
mean(ran.sp$`weighted NODF`)
max(ran.sp$`weighted NODF`)

net.zscore(0.15038761, ran.sp$`weighted connectance`)
mean(ran.sp$`weighted connectance`)


```




#Modules and modularity modelling

{r, module}
library(nnet)
m1 <- multinom(module ~ Species + shrub.density +  Quantity + N.flowers*day, data = all)
summary(m1)
car::Anova(m1)
m2 <- multinom(module ~ Species + shrub.density +  Quantity +day + N.flowers, data = all)
AIC(m1,m2)
car::Anova(m2)
summary(m2)
anova(m1, m2)

#<2 AIC difference, choose simpler
m3 <- multinom(module ~ Species + shrub.density +  Quantity + day, data = all)
AIC(m1, m2, m3)
car::Anova(m3)
summary(m3)

m4 <- multinom(module ~ 1, data = all)
AIC(m1, m2, m3, m4)

kruskal.test(module ~ N.flowers, data = all)
kruskal.test(module ~ shrub.density, data = all)
kruskal.test(module ~ Quantity, data = all)

m4 <- multinom(module ~ Species + shrub.density +  Quantity + imp.den + N.flowers, data = all)
summary(m4)
car::Anova(m4)
AIC(m4)

#make 1000 random networks, project to one-mode, compute modularity
#null.mod <- permatfull(ind1, fixedmar = "both", times = 1000)
#null.mod <- null.mod$perm
#null.one <- map(null.mod, as.one.mode, fill = 0, project = "lower", weighted = TRUE)
#null.module <- map(null.one, netcarto)

#t <- list()  
#for (i in 1:1000){
#    ind <- null.module[[i]][2]
 #   t <- c(t, ind)
#  } 
#t <- unlist(t, use.names=FALSE)
#t <- as.data.frame(t)
#write.csv(t, "randommoduleoutput.csv")
net.zscore(0.335, t$t)
mean(t$t)

print(null.module[[3]][2])


```{r}
#violindles
ggplot(all, aes(module, day)) + geom_violin() + stat_summary(fun.y=mean, geom="point", shape=23, size=2) + geom_jitter(aes(color = Species, shape = Species), position=position_jitter(0.2)) + ylab("Study Day") + xlab("Module") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  + scale_color_discrete(name = "Species", labels = labels) + scale_shape_manual(values=c(3, 16, 17, 3, 16, 17, 3, 16, 17, 3), labels = labels)



labels = c("Acamptopappus sphaerocephalus", "Eriogonum fasciculatum", "Ericameria cooperi", "Ericameria linearifolia", "Larrea tridentata", "Scutellaria mexicana", "Salvia dorri", "Echinocereus engelmannii", "Opuntia basilaris", "Cylindropuntia echinocarpa")

ggplot(all, aes(module, N.flowers)) + geom_boxplot()
ggplot(all, aes(module, shrub.density)) + geom_boxplot()


#centrality species comparisons
#make a smaller dataframe of means and se for figure 1
library(ggthemes)
se <- function(x) {sd(x)/sqrt(length(x))} ## SE


central <- all %>% group_by(Species) %>% summarise(mean.dg = mean(dg1), se.dg = se(dg1), mean.visits = mean(Quantity), se.visits = se(Quantity), mean.eig = mean(V2), se.eig = se(V2), mean.bt = mean(bt1), se.bt = se(bt1))


ggplot(central, aes(Species, mean.visits)) + geom_bar(stat = "identity") + ylab("Visitation Rate")+ geom_errorbar(aes(Species, ymin = mean.visits - se.visits, ymax = mean.visits + se.visits)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 
ggplot(central, aes(Species, mean.dg)) + geom_bar(stat = "identity") + ylab("Degree Centrality")+ geom_errorbar(aes(Species, ymin = mean.dg - se.dg, ymax = mean.dg + se.dg)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

ggplot(central, aes(Species, mean.eig)) + geom_bar(stat = "identity") + ylab("Eigancentrality")+ geom_errorbar(aes(Species, ymin = mean.eig - se.eig, ymax = mean.eig + se.eig)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

ggplot(central, aes(Species, mean.bt)) + geom_bar(stat = "identity") + ylab("Betweenness")+ geom_errorbar(aes(Species, ymin = mean.bt - se.bt, ymax = mean.bt + se.bt)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

ggplot(pl, aes(dg1, prop, color = type)) + geom_point()  +xlab("Degree Centrality")+ geom_smooth(method = lm) + ylab("Proportion of Population") + scale_color_discrete(name = "", labels = c("Conspecific", "Heterospecific")) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

ggplot(pl, aes(shrub.density, prop, color = type)) + geom_point() + geom_smooth(method = lm) +xlab("Shrub Density") + ylab("Proportion of Population") + scale_color_discrete(name = "", labels = c("Conspecific", "Heterospecific")) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 
```