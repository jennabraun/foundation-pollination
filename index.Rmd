...
#value is 0.335, was output by netcarto earlier in script
mean(t$t)
...
m7 <- glmmTMB(Quantity ~ cactus.density + N.flowers * day + (1|Species), family = "nbinom2", data = data)
m8 <- glmmTMB(Quantity ~ con.density + N.flowers * day+ (1|Species), family = "nbinom2", data = data)
mnull <-glmmTMB(Quantity ~ (1|Species), family = "nbinom2", data = data)



m9 <- glmmTMB(Quantity ~ shrub.density + N.flowers * day + (1|Species), family = "nbinom2", data = data)
m10 <- glmmTMB(Quantity ~ N.flowers * day + (1|Species), family = "nbinom2", data = data)
m11 <- glmmTMB(Quantity ~ density + N.flowers * day + (1|Species), family = "nbinom2", data = data)



knitr::kable(anova(m10, m9, m11))
#adding shrub density significantly improves model fits, but adding density does not.

aic.visits <- AIC(m1, m2, m3, m4, m5, m6, m7, m8, m10, m11, mnull)
knitr::kable(AIC(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, mnull))
...
m2 <- glmmTMB(dg1 ~ N.flowers+ Quantity + shrub.density + day + (1|Species), family = "nbinom2", data = all)
m3 <- glmmTMB(dg1 ~ N.flowers+ Quantity + shrub.density + day + (1|Species), family = "poisson", data = all)
m4 <- glmmTMB(dg1 ~ N.flowers+ Quantity + shrub.density + day + (1|Species), family = "nbinom1", data = all)
AIC(m2, m3, m4)
#nbinom2

#check if interaction should be included
m7 <- glmmTMB(dg1 ~ Quantity+ shrub.density + day  + N.flowers + (1|Species), family = "nbinom2", data = all)
summary(m7)
m8 <- glmmTMB(dg1 ~ Quantity+ shrub.density + day  * N.flowers + (1|Species), family = "nbinom2", data = all)

mnull <- glmmTMB(dg1 ~  (1|Species), family = "nbinom2", data = all)

AIC(m1, m2, m3, m7, m8, mnull)

#model without interaction is best
summary(m7)
knitr::kable(car::Anova(m7, type = 2))
check_collinearity(m7)


#differences between species
kruskal.test(dg1 ~ Species, data = all)
ggplot(all, aes(Species, dg1)) + geom_boxplot() + ylab("Degree Centrality")
...
b2 <-glmmTMB(btbin ~ shrub.density + Quantity + day + N.flowers + (1|Species), family = "binomial", data = all)
...
b4 <- glmmTMB(log(bt1) ~ Quantity + shrub.density + day + N.flowers + (1|Species), family = "gaussian", data = bt)
...
```{r, eigancentrality}
ggplot(all, aes(V2)) + geom_density()

v2 <- glmmTMB(V2 ~ Quantity+ shrub.density + N.flowers + day +(1|Species), family = "gaussian", data = all)
...
kruskal.test(V2 ~ Species, data = all)
...
m4 <- glmmTMB(y ~ shrub.density+ dg1 + N.flowers + day +(1|Species), family = "binomial"(link = "logit"), data = all)
summary(m4)
...
m4 <- MASS::glmmPQL(y ~ shrub.density+ dg1+ N.flowers + day, random = ~1|Species, family = "quasibinomial"(link = "logit"), data = all)
...
m1 <- glmmTMB(z ~ day + shrub.density+ dg1+ N.flowers+ (1|Species), family = "binomial"(link = "logit"), data = all)
summary(m1)
check_overdispersion(m1)
#also overdisperse

m5 <- MASS::glmmPQL(z ~ dg1 + shrub.density + N.flowers + day, random = ~1|Species, family = "quasibinomial"(link = "logit"), data = all)
summary(m5)
...
m1 <- glmmTMB(effective.partners ~ Quantity + shrub.density + N.flowers + day +(1|Species), family = "poisson", data = all)

#troubleshooting says no issues as long as no additional warnings

mnull <- glmmTMB(effective.partners ~ (1|Species), family = "poisson", data = all)
...
kruskal.test(effective.partners ~ Species, data = all)
```
...
n1 <- glmmTMB(nestedcontribution ~ N.flowers + Quantity + day + shrub.density + +(1|Species), family = "gaussian", data = all)
shapiro.test(resid(n1))

nnull <- glmmTMB(nestedcontribution ~  (1|Species), family = "gaussian", data = all)
...
m1 <- multinom(module ~ Species + shrub.density +  Quantity +day + N.flowers, data = all)
options(scipen=999)
knitr::kable(car::Anova(m1, type = 2))
...
#run the random modules function on the individual network
...
#ind.ran.allindices <- ranindices(ind1)

#I ran this once and 

ind.H2 <- read.csv("data/Randomization_outputs/indrandomH2.csv")
ind.WC <- read.csv("data/Randomization_outputs/indrandomWC.csv")
ind.WNODF <- read.csv("data/Randomization_outputs/indrandomWNODF.csv")

net.zscore(4.65012620, ind.WNODF$weighted.NODF)
net.zscore(i[10,], ind.WNODF$weighted.NODF)
mean(ind.WNODF$weighted.NODF)
max(ind.WNODF$weighted.NODF)
min(ind.WNODF$weighted.NODF)


net.zscore(i[14,], ind.WC$`weighted connectance`)
mean(ind.WC$`weighted connectance`)
max(ind.WC$`weighted connectance`)
min(ind.WC$`weighted connectance`)

net.zscore(i[19,], ind.H2$H2)
mean(ind.H2$H2)
max(ind.H2$H2)

ran.sp <- ranindices(sp)
net.zscore(n[19,], ran.sp$H2)
mean(ran.sp$H2)
max(ran.sp$H2)

net.zscore(n[10,], ran.sp$`weighted NODF`)
mean(ran.sp$`weighted NODF`)
max(ran.sp$`weighted NODF`)

net.zscore(n[14,], ran.sp$`weighted connectance`)
mean(ran.sp$`weighted connectance`)
```
...
```{r}
#violindles
ggplot(all, aes(module, day)) + geom_violin() + stat_summary(fun.y=mean, geom="point", shape=23, size=2) + geom_jitter(aes(color = Species, shape = Species), position=position_jitter(0.2)) + ylab("Study Day") + xlab("Module") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  + scale_color_discrete(name = "Species", labels = labels) + scale_shape_manual(values=c(3, 16, 17, 3, 16, 17, 3, 16, 17, 3), labels = labels)
...
ggplot(cent, aes(Species, mean)) + geom_bar(stat = "identity", aes(fill = Species)) +  geom_errorbar(aes(Species, ymin = mean - se, ymax = mean + se)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + facet_wrap(~var, scales = "free") 




#ggplot(central, aes(Species, mean.dg)) + geom_bar(stat = "identity") + ylab("Degree Centrality")+ geom_errorbar(aes(Species, ymin = mean - se, ymax = mean + se)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 


#ggplot(central, aes(Species, mean.eig)) + geom_bar(stat = "identity") + ylab("Eigancentrality")+ geom_errorbar(aes(Species, ymin = mean.eig - se.eig, ymax = mean.eig + se.eig)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

#ggplot(central, aes(Species, mean.bt)) + geom_bar(stat = "identity") + ylab("Betweenness")+ geom_errorbar(aes(Species, ymin = mean.bt - se.bt, ymax = mean.bt + se.bt)) + theme_classic() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

ggplot(pl, aes(dg1, prop, color = type)) + geom_point()  +xlab("Degree Centrality")+ geom_smooth(method = lm) + ylab("Proportion of Population") + scale_color_discrete(name = "", labels = c("Conspecific", "Heterospecific")) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

ggplot(pl, aes(shrub.density, prop, color = type)) + geom_point() + geom_smooth(method = lm) +xlab("Shrub Density") + ylab("Proportion of Population") + scale_color_discrete(name = "", labels = c("Conspecific", "Heterospecific")) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 
```
